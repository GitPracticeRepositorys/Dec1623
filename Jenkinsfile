pipeline {
    ...
    stages {
        ...
        stage('Build docker image') {
            steps {
                sh "docker image build -t shivakrishna99/jenkinsdec23workshop:$BUILD_ID ."
            }
        }
        stage('Trivy Scan') {
            steps {
        ...
        stage('deploy to k8s') {
            steps {
                ...
                sh """
                kubectl patch deployment netflix-app -p '{"spec":{"template":{"spec":{"containers":[{"name":"netflix-app","image":"shivakrishna99/jenkinsdec23workshop:$BUILD_ID"}]}}}}'
                """
            }
        }
